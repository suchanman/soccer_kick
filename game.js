/**
 * 3D Soccer Kick Game (Three.js WebGL Engine) - Final Masterpiece
 */
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ============================================================
  // 동적 UI 요소 주입 (고도계, 스피드 라인, 워프, 탭 이펙트)
  // ============================================================
  ...
  // 화면 주입
  const warpEl = document.createElement('div'); warpEl.id = 'warp-overlay'; document.body.appendChild(warpEl);
  const altEl = document.createElement('div'); altEl.id = 'hud-altitude'; altEl.textContent = '고도: 0.0 m'; document.body.appendChild(altEl);
  const speedLinesEl = document.createElement('div'); speedLinesEl.id = 'speed-lines-overlay'; document.body.appendChild(speedLinesEl);
  ...
  try {
    if (typeof firebase !== 'undefined' && firebaseConfig.apiKey !== "YOUR_API_KEY") {
      firebase.initializeApp(firebaseConfig); db = firebase.firestore(); firebaseEnabled = true;
    }
  } catch (e) { console.warn('[Firebase] 초기화 실패:', e.message); }
  ...
  // ============================================================
  // EVENT BANNERS & NICKNAME & RANKING
  // ============================================================
  ...
  function initNickname() {
    if (playerNickname) { if (nicknameInputEl) nicknameInputEl.value = playerNickname; nicknameModalEl?.classList.add('hidden'); } 
    else { nicknameModalEl?.classList.remove('hidden'); }
  }

  function submitNickname() {
    if (!nicknameInputEl) return;
    const val = nicknameInputEl.value.trim();
    if (val.length > 0) { playerNickname = val; localStorage.setItem('soccer_nickname', val); nicknameModalEl?.classList.add('hidden'); } 
    else { nicknameInputEl.style.borderColor = 'rgba(239,68,68,0.8)'; setTimeout(() => { nicknameInputEl.style.borderColor = ''; }, 800); }
  }
  ...
  async function loadRanking() {
    ...
    try {
      ...
      if (snapshot.empty) { rankingListEl.innerHTML = '<p class="ranking-loading">아직 기록이 없습니다.</p>'; return; }

      rankingListEl.innerHTML = '';
      const rankClasses = ['gold', 'silver', 'bronze']; const rankEmojis  = ['🥇', '🥈', '🥉'];
      const seenNicknames = new Set(); let displayRank = 0;

      snapshot.docs.forEach((doc) => {
        const data = doc.data();
        const rawNickname = data.nickname || '익명';
        
        // 환생 뱃지 로직
        const rCount = data.rebirth || 0;
        let badge = rCount > 0 ? '⭐'.repeat(Math.min(rCount, 3)) : '';
        if (rCount >= 5) badge = '🌟'; 
        const displayNickname = rawNickname + (badge ? ' ' + badge : '');

        if (seenNicknames.has(rawNickname) || displayRank >= 10) return;
        seenNicknames.add(rawNickname);

        const isMe = rawNickname === playerNickname;
        ...
        item.innerHTML = `
          ...
          <span class="ranking-nickname">${displayNickname}${isMe ? ' (나)' : ''}</span>
          <span class="ranking-distance">${(data.distance || 0).toFixed(1)} m</span>
        `;
        rankingListEl.appendChild(item);
        displayRank++;
      });
    } catch (e) { rankingListEl.innerHTML = '<p class="ranking-error">⚠️ 랭킹을 불러오지 못했습니다.</p>'; }
  }
  ...
  function playKickSound(pf) {
    ...
    try {
      ...
      osc.type = 'triangle'; osc.frequency.setValueAtTime(100 + pf * 120, audioCtx.currentTime); osc.frequency.exponentialRampToValueAtTime(30, audioCtx.currentTime + 0.3);
  ...
  function playBounceSound() {
    ...
    try {
      const osc = audioCtx.createOscillator(); const gain = audioCtx.createGain();
      osc.type = 'sine'; osc.frequency.setValueAtTime(80, audioCtx.currentTime); osc.frequency.exponentialRampToValueAtTime(35, audioCtx.currentTime + 0.15);
  ...
  function playWhistleSound() {
    ...
    try {
      const osc = audioCtx.createOscillator(); const gain = audioCtx.createGain();
      osc.type = 'sine'; osc.frequency.setValueAtTime(2200, audioCtx.currentTime); osc.frequency.setValueAtTime(2400, audioCtx.currentTime + 0.1); osc.frequency.setValueAtTime(2200, audioCtx.currentTime + 0.2);
  ...
  const rbBtn = document.createElement('button'); rbBtn.className = 'btn-rebirth-hud'; rbBtn.textContent = '🌌 환생 스탯';
  ...
  const crystalDisp = document.createElement('div'); crystalDisp.id = 'crystal-display'; document.body.appendChild(crystalDisp);

  const rbModal = document.createElement('div'); rbModal.id = 'rebirth-modal'; rbModal.className = 'modal-overlay hidden';
  rbModal.innerHTML = `
  ...
  function updateRebirthUI() {
    crystalDisp.textContent = `💎 결정: ${crystals}개`; document.getElementById('modal-crystals').textContent = crystals;
  ...
  ufoGroup.position.set(15, 500, 0); 
  scene.add(ufoGroup);
  ...
  satGroup.position.set(-15, 1000, 0); 
  scene.add(satGroup);
  ...
  function createGrassTexture() {
    ...
    const tex = new THREE.CanvasTexture(c); tex.wrapS = tex.wrapT = THREE.RepeatWrapping; tex.repeat.set(10, 10); return tex;
  }
  function createSoccerBallTexture() {
    const c = document.createElement('canvas'); c.width = 512; c.height = 256; const ctx = c.getContext('2d');
    ctx.fillStyle = '#ffffff'; ctx.fillRect(0, 0, 512, 256); ctx.fillStyle = '#0f172a';
    [[64,64],[192,64],[320,64],[448,64],[128,192],[256,192],[384,192]].forEach(([cx,cy]) => {
      ctx.beginPath(); for (let i = 0; i < 5; i++) { const a = i * Math.PI*2/5 - Math.PI/2; const x = cx + Math.cos(a)*28, y = cy + Math.sin(a)*28; i === 0 ? ctx.moveTo(x,y) : ctx.lineTo(x,y); } ctx.closePath(); ctx.fill();
    });
    return new THREE.CanvasTexture(c);
  }
  function createJerseyTexture() {
    const c = document.createElement('canvas'); c.width = c.height = 256; const ctx = c.getContext('2d');
    ctx.fillStyle = '#dc2626'; ctx.fillRect(0, 0, 256, 256); ctx.fillStyle = '#ffffff'; ctx.font = '900 110px Pretendard, sans-serif'; ctx.textAlign = 'center'; ctx.fillText('10', 128, 160); return new THREE.CanvasTexture(c);
  }
  ...
  const bannerMatCache = new Map();

  const startGround = new THREE.Mesh(new THREE.PlaneGeometry(60, 20), groundMaterial);
  startGround.rotation.x = -Math.PI / 2; startGround.position.set(0, 0, 5); startGround.receiveShadow = true; scene.add(startGround);
  ...
  function getBannerMaterial(dist) {
    ...
    bCtx.fillStyle = '#ef4444'; bCtx.fillRect(0, 0, 256, 128); bCtx.strokeStyle = '#fde047'; bCtx.lineWidth = 10; bCtx.strokeRect(0, 0, 256, 128);
    bCtx.fillStyle = '#ffffff'; bCtx.font = '900 48px Pretendard, sans-serif'; bCtx.textAlign = 'center';
    if (dist >= 1000) bCtx.fillText((dist / 1000).toFixed(1) + 'km', 128, 80); else bCtx.fillText(dist + 'm', 128, 80);
    const bannerTex = new THREE.CanvasTexture(bc); const bannerMat = new THREE.MeshStandardMaterial({ map: bannerTex, side: THREE.DoubleSide }); bannerMatCache.set(dist, bannerMat); return bannerMat;
  }

  function createChunk(chunkIndex) {
    if (loadedChunks.has(chunkIndex)) return;
    const meshes = []; const startDist = chunkIndex * CHUNK_SIZE; const centerZ = -(startDist + CHUNK_SIZE / 2);
    const groundMesh = new THREE.Mesh(sharedGroundGeo, groundMaterial); groundMesh.rotation.x = -Math.PI / 2; groundMesh.position.set(0, 0, centerZ); groundMesh.receiveShadow = true; scene.add(groundMesh); meshes.push(groundMesh);
    for (let dist = startDist + 500; dist <= startDist + CHUNK_SIZE; dist += 500) {
      const zPos = -dist;
      const poleMesh = new THREE.Mesh(sharedPoleGeo, poleMaterial); poleMesh.position.set(-8.0, 2.0, zPos); poleMesh.castShadow = true; scene.add(poleMesh); meshes.push(poleMesh);
      const bannerMat = getBannerMaterial(dist); const bannerMesh = new THREE.Mesh(sharedBannerGeo, bannerMat); bannerMesh.position.set(-6.7, 3.2, zPos); bannerMesh.castShadow = false; bannerMesh.receiveShadow = false; scene.add(bannerMesh); meshes.push(bannerMesh);
    }
    loadedChunks.set(chunkIndex, meshes);
  }
  function removeChunk(chunkIndex) { const meshes = loadedChunks.get(chunkIndex); if (!meshes) return; meshes.forEach(m => scene.remove(m)); loadedChunks.delete(chunkIndex); }
  function updateChunks() {
    const ballDistM  = Math.abs(ballMesh.position.z); const curChunk   = Math.floor(ballDistM / CHUNK_SIZE);
    const keepFrom   = Math.max(0, curChunk - 1); const keepTo     = curChunk + 2;
    for (let i = keepFrom; i <= keepTo; i++) createChunk(i);
    for (const [idx] of loadedChunks) { if (idx < keepFrom || idx > keepTo) removeChunk(idx); }
  }
  function initChunks() { for (const [idx] of loadedChunks) removeChunk(idx); createChunk(0); createChunk(1); createChunk(2); }

  // ============================================================
  // SOCCER BALL & SKINS
  // ============================================================
  const BALL_RADIUS = 0.35; const ballMesh = new THREE.Group(); ballMesh.position.set(0, BALL_RADIUS, 0); scene.add(ballMesh);
  const basicBall = new THREE.Mesh(new THREE.SphereGeometry(BALL_RADIUS, 32, 32), new THREE.MeshStandardMaterial({ map: createSoccerBallTexture(), roughness: 0.3, metalness: 0.1 })); basicBall.castShadow = true; basicBall.receiveShadow = true;
  function createRainbowTexture() { const c = document.createElement('canvas'); c.width = 256; c.height = 256; const ctx = c.getContext('2d'); const grad = ctx.createLinearGradient(0,0,256,0); ['#ef4444','#f97316','#eab308','#22c55e','#3b82f6','#a855f7'].forEach((col, i, arr) => grad.addColorStop(i/(arr.length-1), col)); ctx.fillStyle = grad; ctx.fillRect(0,0,256,256); return new THREE.CanvasTexture(c); }
  const rainbowBall = new THREE.Mesh(new THREE.SphereGeometry(BALL_RADIUS, 32, 32), new THREE.MeshStandardMaterial({ map: createRainbowTexture(), roughness: 0.2, metalness: 0.3 })); rainbowBall.castShadow = true; rainbowBall.receiveShadow = true;
  ...
  // ============================================================
  // PLAYER MODEL & EVENT MESHES
  // ============================================================
  const playerGroup = new THREE.Group(); playerGroup.position.set(0.65, 0, 0.45); playerGroup.rotation.y = -Math.PI * 0.15;
  const jerseyMat = new THREE.MeshStandardMaterial({ map: createJerseyTexture(), roughness: 0.6 }); const shortsMat = new THREE.MeshStandardMaterial({ color: '#ffffff', roughness: 0.5 }); const skinMat   = new THREE.MeshStandardMaterial({ color: '#fca5a5', roughness: 0.7 }); const sockMat   = new THREE.MeshStandardMaterial({ color: '#dc2626', roughness: 0.6 }); const shoeMat   = new THREE.MeshStandardMaterial({ color: '#1e3a8a', roughness: 0.4 }); const hairMat   = new THREE.MeshStandardMaterial({ color: '#1e1b4b', roughness: 0.8 });
  const torsoMesh = new THREE.Mesh(new THREE.BoxGeometry(0.55, 0.75, 0.3), jerseyMat); torsoMesh.position.set(0, 1.25, 0); torsoMesh.castShadow = true; playerGroup.add(torsoMesh);
  const shortsMesh = new THREE.Mesh(new THREE.BoxGeometry(0.57, 0.35, 0.32), shortsMat); shortsMesh.position.set(0, 0.82, 0); shortsMesh.castShadow = true; playerGroup.add(shortsMesh);
  const headMesh = new THREE.Mesh(new THREE.SphereGeometry(0.2, 16, 16), skinMat); headMesh.position.set(0, 1.8, 0); headMesh.castShadow = true; playerGroup.add(headMesh);
  const hairMesh = new THREE.Mesh(new THREE.SphereGeometry(0.21, 16, 16, 0, Math.PI*2, 0, Math.PI*0.5), hairMat); hairMesh.position.set(0, 1.83, 0); hairMesh.castShadow = true; playerGroup.add(hairMesh);
  const legGeo  = new THREE.CylinderGeometry(0.09, 0.08, 0.65, 12); const shoeGeo = new THREE.BoxGeometry(0.14, 0.12, 0.3);
  ...
  const leftShoeMesh = new THREE.Mesh(shoeGeo, shoeMat); leftShoeMesh.position.set(0, -0.58, -0.06); leftShoeMesh.castShadow = true; leftLegPivot.add(leftShoeMesh); playerGroup.add(leftLegPivot);
  const armGeo   = new THREE.CylinderGeometry(0.07, 0.06, 0.6, 12); const leftArm  = new THREE.Mesh(armGeo, skinMat); leftArm.position.set(-0.35, 1.25, 0); leftArm.rotation.z = 0.2; leftArm.castShadow = true; playerGroup.add(leftArm); const rightArm = new THREE.Mesh(armGeo, skinMat); rightArm.position.set(0.35, 1.25, 0); rightArm.rotation.z = -0.2; rightArm.castShadow = true; playerGroup.add(rightArm);
  scene.add(playerGroup);

  const jetpackGroup = new THREE.Group(); const packMat = new THREE.MeshStandardMaterial({ color: '#ef4444', metalness: 0.8, roughness: 0.2 }); [[-0.2, 0, 0], [0.2, 0, 0]].forEach(([x, y, z]) => { const m = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.12, 0.6, 12), packMat); m.position.set(x, y, z); jetpackGroup.add(m); }); const packFrame = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.2, 0.1), new THREE.MeshStandardMaterial({ color: '#334155' })); packFrame.position.set(0, 0.1, 0); jetpackGroup.add(packFrame); jetpackGroup.visible = false; scene.add(jetpackGroup);
  const airplaneGroup = new THREE.Group(); const planeBody = new THREE.Mesh(new THREE.CylinderGeometry(0.4, 0.2, 3.5, 12), new THREE.MeshStandardMaterial({ color: '#f8fafc', metalness: 0.8, roughness: 0.2 })); planeBody.rotation.z = Math.PI / 2; airplaneGroup.add(planeBody); const planeWing = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.08, 4.0), new THREE.MeshStandardMaterial({ color: '#2563eb', metalness: 0.6, roughness: 0.3 })); planeWing.position.set(0, 0.1, 0); airplaneGroup.add(planeWing); const planeTail = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.8, 0.08), new THREE.MeshStandardMaterial({ color: '#ef4444' })); planeTail.position.set(-1.4, 0.5, 0); airplaneGroup.add(planeTail); airplaneGroup.visible = false; scene.add(airplaneGroup);
  const eagleGroup = new THREE.Group(); const eagleBody = new THREE.Mesh(new THREE.ConeGeometry(0.3, 1.2, 8), new THREE.MeshStandardMaterial({ color: '#78350f', roughness: 0.8 })); eagleBody.rotation.x = Math.PI / 2; eagleGroup.add(eagleBody); [[-1.1, 0.1, 0], [1.1, 0.1, 0]].forEach(([x, y, z]) => { const w = new THREE.Mesh(new THREE.BoxGeometry(2.2, 0.06, 0.6), new THREE.MeshStandardMaterial({ color: '#451a03' })); w.position.set(x, y, z); eagleGroup.add(w); }); const eagleBeak = new THREE.Mesh(new THREE.ConeGeometry(0.1, 0.3, 6), new THREE.MeshStandardMaterial({ color: '#f59e0b' })); eagleBeak.rotation.x = Math.PI / 2; eagleBeak.position.set(0, 0, -0.75); eagleGroup.add(eagleBeak); eagleGroup.visible = false; scene.add(eagleGroup);
  const windParticlesGroup = new THREE.Group(); for (let i = 0; i < 30; i++) { const streak = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.02, 3.0, 6), new THREE.MeshBasicMaterial({ color: '#60a5fa', transparent: true, opacity: 0.7 })); streak.rotation.x = Math.PI / 2; streak.position.set((Math.random()-0.5)*4, Math.random()*3+0.5, (Math.random()-0.5)*6); windParticlesGroup.add(streak); } windParticlesGroup.visible = false; scene.add(windParticlesGroup);
  const rocketGroup = new THREE.Group(); const rocketCone = new THREE.Mesh(new THREE.ConeGeometry(0.35, 0.8, 16), new THREE.MeshStandardMaterial({ color: '#ef4444' })); rocketCone.position.set(0, 1.2, 0); rocketGroup.add(rocketCone); const rocketBody = new THREE.Mesh(new THREE.CylinderGeometry(0.35, 0.35, 1.6, 16), new THREE.MeshStandardMaterial({ color: '#f8fafc', metalness: 0.6, roughness: 0.3 })); rocketBody.position.set(0, 0, 0); rocketGroup.add(rocketBody); const fin1 = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.5, 0.6), new THREE.MeshStandardMaterial({ color: '#2563eb' })); fin1.position.set(0, -0.6, 0); rocketGroup.add(fin1); rocketGroup.visible = false; scene.add(rocketGroup);
  const moleGroup = new THREE.Group(); const mound = new THREE.Mesh(new THREE.CylinderGeometry(0.6, 0.8, 0.3, 16), new THREE.MeshStandardMaterial({ color: '#78350f', roughness: 0.9 })); mound.position.set(0, 0.15, 0); moleGroup.add(mound); const moleHead = new THREE.Mesh(new THREE.SphereGeometry(0.4, 16, 16), new THREE.MeshStandardMaterial({ color: '#92400e', roughness: 0.8 })); moleHead.position.set(0, 0.45, 0); moleGroup.add(moleHead); const moleNose = new THREE.Mesh(new THREE.SphereGeometry(0.1, 12, 12), new THREE.MeshStandardMaterial({ color: '#f472b6' })); moleNose.position.set(0, 0.5, 0.35); moleGroup.add(moleNose); moleGroup.visible = false; scene.add(moleGroup);
  ...
  function clearAllTrailParticles() { for (let i = trailParticles.length - 1; i >= 0; i--) { const tp = trailParticles[i]; trailGroup.remove(tp.mesh); if (trailPool[tp.mesh.userData.type]) trailPool[tp.mesh.userData.type].push(tp.mesh); } trailParticles.length = 0; }

  // ============================================================
  // GAME STATE
  // ============================================================
  const STATES = { IDLE: 'IDLE', CHARGING: 'CHARGING', KICKING: 'KICKING', FLYING: 'FLYING', STOPPED: 'STOPPED' };
  let gameState = STATES.IDLE; let power = 0; const POWER_SPEED = 140;
  let ballVel = { x: 0, y: 0, z: 0 }; let ballRot = { x: 0 }; let isGrounded = true; let kickAnimProgress = 0; let isFireballMode = false; let hasTouchedGround = false;

  let alt200Triggered = false; let alt500Triggered = false; let alt1000Triggered = false;
 ...
 let isJetpackAttached = false, isJetpackDetached = false, jetpackVelY = 0, airplaneActive = false, airplaneProgress = 0, isEagleCarrying = false, eagleTimer = 0, isRocketPushing = false, rocketTimer = 0;
  ...
  function resetGame() {
    ...
    ballMesh.position.set(0, BALL_RADIUS, 0); ballMesh.rotation.set(0, 0, 0); ballVel = { x: 0, y: 0, z: 0 }; ballRot = { x: 0 }; isGrounded = true; leftLegPivot.rotation.x = 0; kickAnimProgress = 0; isFireballMode = false; hasTouchedGround = false; clearAllTrailParticles();
    
    alt200Triggered = alt500Triggered = alt1000Triggered = false; speedLinesEl.style.opacity = 0; scene.background = new THREE.Color('#87ceeb'); scene.fog.color = scene.background; scene.fog.density = 0.0025; starsMat.opacity = 0; camera.fov = 55; camera.updateProjectionMatrix(); if (altEl) altEl.textContent = '고도: 0.0 m';
    ...
    if (startInstructionEl) { if (!localStorage.getItem('soccer_tutorial_seen')) startInstructionEl.classList.remove('fade-out'); else startInstructionEl.classList.add('fade-out'); }
    resultModalEl?.classList.add('hidden'); if (eventBannerContainer) eventBannerContainer.innerHTML = '';
    speedMultiplier = 1; if (speedBtnEl) { speedBtnEl.textContent = '▶▶ 1x'; speedBtnEl.classList.remove('active'); }
    initChunks();
  }

  function handlePressStart() { if (gameState === STATES.STOPPED) return; initAudio(); if (gameState === STATES.IDLE) { gameState = STATES.CHARGING; startInstructionEl?.classList.add('fade-out'); localStorage.setItem('soccer_tutorial_seen', 'true'); } }
  function handlePressEnd() { if (gameState === STATES.CHARGING) gameState = STATES.KICKING; }
  kickBtnEl?.addEventListener('pointerdown', handlePressStart); kickBtnEl?.addEventListener('pointerup', handlePressEnd); kickBtnEl?.addEventListener('pointerleave', () => { if (gameState === STATES.CHARGING) handlePressEnd(); });
  window.addEventListener('keydown', (e) => { if (e.code === 'Space' && !e.repeat) handlePressStart(); }); window.addEventListener('keyup', (e) => { if (e.code === 'Space') handlePressEnd(); }); restartBtn?.addEventListener('click', resetGame);

  function updatePowerUI() { const p = Math.min(Math.max(Math.floor(power), 0), 100); if (powerNumberEl) powerNumberEl.textContent = p + '%'; if (powerBarEl) { powerBarEl.style.width = p + '%'; powerBarEl.style.boxShadow = p > 85 ? '0 0 20px rgba(255,0,85,0.9)' : p > 60 ? '0 0 15px rgba(234,179,8,0.7)' : '0 0 10px rgba(34,197,94,0.5)'; } }
  ...
  function triggerBallLaunch() {
    ...
    hasJetpackEvent  = Math.random() < 0.5; hasAirplaneEvent = Math.random() < 0.5; hasEagleEvent    = Math.random() < 0.5; hasWindEvent     = Math.random() < 0.5; hasRocketEvent   = Math.random() < 0.5; hasMoleEvent     = Math.random() < 0.5; hasHeadwindEvent = Math.random() < 0.3; hasSecondKickEvent = Math.random() < 0.5;
  ...
  function updatePhysics(dt) {
    ...
    if (gameState === STATES.FLYING) {
      ballVel.y -= 25.0 * dt; const altitude = Math.max(0, ballMesh.position.y); const airDrag = Math.min(0.9995, 0.996 + (altitude * 0.00002)); 
      ...
      if (currentSkin === 'bullet') { if (ballVel.y !== 0 || ballVel.z !== 0) ballMesh.lookAt(ballMesh.position.x + ballVel.x, ballMesh.position.y + ballVel.y, ballMesh.position.z + ballVel.z); } else { ballMesh.rotation.x += ballRot.x * dt; }

      const cZ = ballMesh.position.z;
      if (hasJetpackEvent && !cpJetpackTriggered && cZ <= -(totalTargetDistance * 0.25)) { cpJetpackTriggered = true; showEventBanner('🚀', `JETPACK BOOST! +${eventBonus}m`); totalTargetDistance += eventBonus; ballVel.z -= 32.0 * eventBonusVelScale; ballVel.y += 18.0 * eventBonusVelScale; isJetpackAttached = true; jetpackGroup.visible = true; }
      if (isJetpackAttached) { jetpackGroup.position.copy(ballMesh.position); jetpackGroup.position.z += 0.2; if (ballVel.y < 0 && !isJetpackDetached) { isJetpackAttached = false; isJetpackDetached = true; jetpackVelY = -2.0; } }
      if (isJetpackDetached && jetpackGroup.visible) { jetpackVelY -= 15.0 * dt; jetpackGroup.position.y += jetpackVelY * dt; jetpackGroup.rotation.z += dt * 3; if (jetpackGroup.position.y <= 0) jetpackGroup.visible = false; }
      ...
      if (isGrounded && Math.abs(ballVel.z) < 0.3) {
        if (!cpMoleTriggered) { cpMoleTriggered = true; if (hasMoleEvent) { showEventBanner('🦔', `MOLE BOUNCE! +${eventBonus}m`); totalTargetDistance += eventBonus; moleGroup.position.set(ballMesh.position.x, 0, ballMesh.position.z); moleGroup.visible = true; ballVel.z = -32.0 * eventBonusVelScale; ballVel.y = 16.0 * eventBonusVelScale; isGrounded = false; return; } }
  ...
  function updateVisuals() {
    ...
    for (let i = trailParticles.length - 1; i >= 0; i--) {
      ...
      if (tp.life <= 0) { trailGroup.remove(tp.mesh); if (trailPool[tp.mesh.userData.type]) trailPool[tp.mesh.userData.type].push(tp.mesh); trailParticles.splice(i, 1); }
    }
    ...
    cloudsGroup.position.z = ballMesh.position.z; ufoGroup.position.z = ballMesh.position.z - 20; ufoGroup.rotation.y += 0.02; satGroup.position.z = ballMesh.position.z - 10; satGroup.rotation.x += 0.01;

    const skyCol = new THREE.Color('#87ceeb'); const midCol = new THREE.Color('#1e3a8a'); const spaceCol = new THREE.Color('#000000');
    if (alt < 300) { const t = Math.min(alt / 300, 1.0); scene.background = skyCol.lerp(midCol, t); starsMat.opacity = 0; if(altEl) altEl.style.color = '#38bdf8'; } 
    else { const t = Math.min((alt - 300) / 700, 1.0); scene.background = midCol.lerp(spaceCol, t); starsMat.opacity = t; if(altEl) altEl.style.color = '#fde047'; }
    scene.fog.color = scene.background; scene.fog.density = 0.0025 * Math.max(0, 1 - (alt/400));
    ...
    if (speedLinesEl && gameState === STATES.FLYING) { let spdOp = 0; if (verticalSpeed > 15) spdOp = Math.min((verticalSpeed - 15) / 30, 0.7); speedLinesEl.style.opacity = spdOp; } else if (speedLinesEl) { speedLinesEl.style.opacity = 0; }

    if (gameState === STATES.FLYING || gameState === STATES.STOPPED) {
      const tCam = Math.min(alt / 400, 1.0); 
      camera.fov = THREE.MathUtils.lerp(camera.fov, 55 + (tCam * 25), 0.1); camera.updateProjectionMatrix();
      targetCamPos.set(ballMesh.position.x * 0.5 + 0.3, Math.max(ballMesh.position.y + 2.4, 2.5), ballMesh.position.z + 5.5);
      const lookYOffset = THREE.MathUtils.lerp(0.5, -15.0, tCam); targetCamLookAt.set(ballMesh.position.x, ballMesh.position.y + lookYOffset, ballMesh.position.z - 4.0);
      if (gameState === STATES.FLYING && speedMultiplier === 50 && !hasTouchedGround) { warpEl.style.opacity = 0.8; targetCamPos.x += (Math.random() - 0.5) * 1.5; targetCamPos.y += (Math.random() - 0.5) * 1.5; } else { warpEl.style.opacity = 0; }
      camera.position.lerp(targetCamPos, 0.08); camera.lookAt(targetCamLookAt); dirLight.position.set(ballMesh.position.x + 20, 40, ballMesh.position.z + 20);
    } else { camera.fov = THREE.MathUtils.lerp(camera.fov, 55, 0.1); camera.updateProjectionMatrix(); camera.position.lerp(defaultCamPos, 0.1); camera.lookAt(defaultCamTarget); }
  }

  function handleGameOver(finalDistance) {
    playWhistleSound();
    const baseEarned = Math.floor(finalDistance); const earned = Math.floor(baseEarned * (1 + (lvCoin * 0.1)) * (1 + (lvSuperCoin * 5.0)));
    coins += earned; localStorage.setItem('soccer_coins', coins.toString());

    let isNewBest = false;
    if (finalDistance > bestDistance) { bestDistance = finalDistance; localStorage.setItem('soccer_3d_best_distance', bestDistance.toString()); if (bestDistanceEl) bestDistanceEl.textContent = bestDistance.toFixed(1) + ' m'; isNewBest = true; }
    if (finalDistanceEl) finalDistanceEl.textContent = finalDistance.toFixed(1) + ' m'; if (earnedCoinsEl) earnedCoinsEl.textContent = '+' + earned.toLocaleString(); updateCurrencyUI();
    if (resultBadgeEl && resultTitleEl) { resultBadgeEl.style.display = 'inline-block'; if (isNewBest) { resultBadgeEl.textContent = 'NEW BEST RECORD! 🏆'; resultTitleEl.textContent = 'WORLD CLASS!'; } else if (finalDistance > 300) { resultBadgeEl.textContent = 'SUPER KICK! ⭐'; resultTitleEl.textContent = 'INCREDIBLE!'; } else { resultBadgeEl.style.display = 'none'; resultTitleEl.textContent = 'GREAT KICK!'; } }
    resultModalEl?.classList.remove('hidden');
    if (firebaseEnabled && playerNickname) { if (isNewBest) { db.collection('soccer_scores').doc(playerNickname).set({ nickname: playerNickname, distance: parseFloat(finalDistance.toFixed(2)), rebirth: rebirthCount, timestamp: firebase.firestore.FieldValue.serverTimestamp() }).catch(e => console.warn('[Firebase] 기록 저장 실패:', e.message)); } }
  }

  window.addEventListener('pointerdown', (e) => {
    if (e.target.closest('button') || e.target.closest('.modal')) return;
    if (gameState === STATES.FLYING && !hasTouchedGround) {
      const bonusCoin = speedMultiplier === 50 ? 50 : 1; coins += bonusCoin; updateCurrencyUI();
      ballVel.z -= 1.0 * speedMultiplier; ballVel.y += 0.5 * speedMultiplier;
      const ripple = document.createElement('div'); ripple.className = 'tap-ripple'; ripple.style.left = e.clientX + 'px'; ripple.style.top = e.clientY + 'px';
      const coinText = document.createElement('div'); coinText.className = 'tap-coin'; coinText.textContent = '+' + bonusCoin; coinText.style.left = e.clientX + 'px'; coinText.style.top = (e.clientY - 20) + 'px';
      document.body.appendChild(ripple); document.body.appendChild(coinText); setTimeout(() => { ripple.remove(); coinText.remove(); }, 600);
    }
  });

  window.addEventListener('resize', () => { camera.aspect = window.innerWidth / window.innerHeight; camera.updateProjectionMatrix(); renderer.setSize(window.innerWidth, window.innerHeight); });

  const FIXED_DT = 1 / 60; let accumulator = 0; let lastTime = performance.now();
  function animate(now) {
    requestAnimationFrame(animate);
    let dt = (now - lastTime) / 1000; if (dt > 0.1) dt = 0.1; lastTime = now;
    if (gameState === STATES.FLYING || gameState === STATES.KICKING) { dt *= speedMultiplier; }
    accumulator += dt; if (accumulator > 2.0) accumulator = 2.0;
    let steps = 0;
    while (accumulator >= FIXED_DT) { updatePhysics(FIXED_DT); accumulator -= FIXED_DT; steps++; if (steps >= 120) { accumulator = 0; break; } }
    updateVisuals(); updateChunks(); renderer.render(scene, camera);
  }

  initNickname(); resetGame(); requestAnimationFrame(animate);
});
